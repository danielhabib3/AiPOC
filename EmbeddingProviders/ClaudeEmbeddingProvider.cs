...
{
    public class ClaudeEmbeddingProvider(ClaudeConfiguration llmConfig, string collectionPrefixName) : EmbeddingProvider(llmConfig, EmbeddingProviderType.Claude, collectionPrefixName)
    {
        public override async Task<List<float>?> GetEmbeddingAsync(HttpClient client, string text)
        {
            ...
            var requestBody = new { input = new[] { text }, model = this.ModelName };
