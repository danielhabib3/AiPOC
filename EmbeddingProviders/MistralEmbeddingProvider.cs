...
{
    public class MistralEmbeddingProvider(MistralConfiguration llmConfig, string collectionPrefixName) : EmbeddingProvider(llmConfig, EmbeddingProviderType.Mistral, collectionPrefixName)
    {
        public override async Task<List<float>?> GetEmbeddingAsync(HttpClient client, string text)
        {
            ...
            var requestBody = new { model = this.ModelName, input = new[] { text } };
